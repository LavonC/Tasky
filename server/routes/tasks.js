...
export default function taskRoutes(pool) {
  ...
  router.post('/', async (req, res) => {
    try {
      ...
      await pool.execute(
        `UPDATE project SET progress = ? WHERE id = ?`,
        [progress, project_id]
      );
  ...
  router.put('/:id', async (req, res) => {
    try {
      ...
      if (assignee_ids !== undefined) {
        // Deactivate current assignments - handle duplicates
        try {
          await pool.execute(
          ...
          // Ignore duplicate key errors, continue with new assignments
          console.log('Warning during assignment deactivation:', err.message);
        }

        // Insert new ones - handle duplicates
        for (const userId of assignee_ids) {
          try {
            await pool.execute(
              'INSERT INTO task_assignment (task_id, user_id, assigned_by) VALUES (?, ?, ?)',
  ...
  router.post('/:id/subtasks', async (req, res) => {
    try {
      ...
      res.json({ success: true, subtask: newSubtask[0] });
    } catch (error) {
  ...
  // PUT /api/pm/tasks/:id/subtasks/:subtaskId
  router.put('/:id/subtasks/:subtaskId', async (req, res) => {
    try {
      ...
      await pool.execute(
        `UPDATE subtask SET ${updates.join(', ')} WHERE id = ? AND task_id = ?`,
        values
      );

      const [updatedSubtask] = await pool.execute('SELECT * FROM subtask WHERE id = ?', [subtaskId]);
      res.json({ success: true, subtask: updatedSubtask[0] });
    } catch (error) {
  ...
  // DELETE /api/pm/tasks/:id/subtasks/:subtaskId
  router.delete('/:id/subtasks/:subtaskId', async (req, res) => {
    try {
      const taskId = req.params.id;
      const subtaskId = req.params.subtaskId;

      await pool.execute('DELETE FROM subtask WHERE id = ? AND task_id = ?', [subtaskId, taskId]);
      res.json({ success: true });
    } catch (error) {
