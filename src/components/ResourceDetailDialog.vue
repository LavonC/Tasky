              ...
              >
                ...
                >
                  <template v-slot:header>
                    ...
                    <q-item-section>
                      ...
                      <q-item-label caption class="row items-center q-mt-xs">
                        ...
                        <q-badge v-if="task.is_overdue" color="red" label="Overdue" class="q-ml-sm" style="font-size: 10px" />
                      </q-item-label>
                      <q-item-label caption class="row items-center q-mt-xs">
                        <span class="q-mr-sm">Review: </span>
                        <q-badge
                          :color="getReviewStatusColor(task.review_status)"
                          :label="formatReviewStatus(task.review_status)"
                          style="font-size: 10px"
                        />
                      </q-item-label>
                    </q-item-section>
                  ...
                  <q-card flat class="bg-grey-1 q-pa-md">
                    ...
                    <!-- Review Comments -->
                    <div v-if="task.review_comment || task.completion_comment" class="q-mb-md">
                      <div class="text-subtitle2 text-weight-bold q-mb-sm">Review Comments</div>
                      <div v-if="task.review_comment" class="bg-white q-pa-sm rounded-borders q-mb-sm">
                        <div class="text-caption text-grey-7">Reviewer Comment:</div>
                        <div class="text-body2">{{ task.review_comment }}</div>
                      </div>
                      <div v-if="task.completion_comment" class="bg-white q-pa-sm rounded-borders">
                        <div class="text-caption text-grey-7">Completion Comment:</div>
                        <div class="text-body2">{{ task.completion_comment }}</div>
                      </div>
                    </div>
                    ...
                    <div v-if="task.subtasks && task.subtasks.length > 0">
                      ...
                      <q-list separator class="bg-white rounded-borders">
                        <q-item v-for="subtask in task.subtasks" :key="subtask.id">
                          <q-item-section avatar>
                            <q-checkbox
                              :model-value="subtask.completed === 1 || subtask.status === 'completed'"
                              disable
                              color="green"
                            />
                          </q-item-section>
                          <q-item-section>
                            <q-item-label :class="{ 'text-grey-6': subtask.completed === 1 || subtask.status === 'completed' }">
                              {{ subtask.title }}
                            </q-item-label>
                            <q-item-label caption>
                              Progress: {{ subtask.progress || 0 }}%
                              <span v-if="subtask.deadline" class="q-ml-sm">• Due: {{ formatDate(subtask.deadline) }}</span>
                            </q-item-label>
                          </q-item-section>
                          <q-item-section side>
                            <q-badge
                              :color="subtask.completed === 1 || subtask.status === 'completed' ? 'green' : 'grey'"
        ...
        <q-card-section class="q-pt-none">
          ...
          <q-list v-else separator>
            <q-item
              ...
              :class="{ 'bg-orange-1': selectedNewEmployee && selectedNewEmployee.user_id === emp.user_id }"
            >
              <q-item-section avatar>
                <q-avatar size="32px">
                  <img :src="emp.avatar_url || `https://i.pravatar.cc/150?img=${emp.user_id}`" />
                </q-avatar>
              </q-item-section>
              <q-item-section>
...
const getReviewStatusColor = (reviewStatus: string) => {
  ...
  return map[reviewStatus] || 'grey';
};

const formatReviewStatus = (reviewStatus: string) => {
  if (!reviewStatus) return 'Not Submitted';
  return reviewStatus
    .split('-')
    .map((w) => w.charAt(0).toUpperCase() + w.slice(1))
    .join(' ');
  ...
  try {
    ...
    if (data.success) {
      ...
      eligibleEmployees.value = data.resources.filter(
        (r: any) => r.user_id !== props.resourceId
      );
    }
  } catch (error: any) {
...
const executeReassignment = async () => {
  ...
  try {
    const payload = {
      ...
      toUserId: selectedNewEmployee.value.user_id,
    };
    ...
    if (data.success) {
      ...
      await resourceStore.fetchResourceById(props.resourceId.toString());
      // Emit event to refresh parent workload data
      emit('reassigned');
    } else {
      $q.notify({
        type: 'negative',
