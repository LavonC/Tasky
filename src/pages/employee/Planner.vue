...
const reviewerOptions = computed(() => {
  console.log('=== PLANNER REVIEWER OPTIONS ===');
  console.log('Planner employees:', employees.value);
  console.log('Planner employees length:', employees.value?.length);
  console.log('Planner user:', authStore.user);
  console.log('Planner user ID:', authStore.user?.id, 'Type:', typeof authStore.user?.id);

  if (!employees.value || employees.value.length === 0) {
    console.log('❌ Planner: No employees loaded');
    return [];
  }
  ...
  console.log('✅ Planner reviewer options:', options);
  console.log('=== END PLANNER REVIEWER OPTIONS ===');
  return options;
});
