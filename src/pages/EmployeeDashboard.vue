    ...
    const response = await fetch(
      `http://localhost:3007/api/employee/tasks/${selectedTask.value.id}`,
      {
        ...
        body: JSON.stringify({
          ...
          user_id: currentEmployee.value?.id || 36, // Use current employee ID or default to Jiya's ID
        }),
      },
    );
...
const reviewerOptions = computed(() => {
  console.log('=== REVIEWER OPTIONS COMPUTED ===');
  console.log('Employees array:', employees.value);
  console.log('Employees length:', employees.value?.length);
  console.log('Current Employee:', currentEmployee.value);
  console.log('Current Employee ID:', currentEmployee.value?.id, 'Type:', typeof currentEmployee.value?.id);

  if (!employees.value || employees.value.length === 0) {
    console.log('❌ No employees loaded');
    return [];
  }

  const currentId = Number(currentEmployee.value?.id);
  ...
  // Return objects with label (Name — Email) and value for proper select binding
  ...
  console.log('✅ Final reviewer options:', options);
  console.log('=== END REVIEWER OPTIONS ===');
  return options;
});
